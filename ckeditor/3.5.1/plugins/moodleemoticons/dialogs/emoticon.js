...
CKEDITOR.dialog.add( 'emoticon', function( editor )
{
	var config = editor.config,
		lang = editor.lang.smiley,
		emoticons = config.emoticons,// || M.editor_ckeditor.emoticons,
		//images = config.smiley_images,
		//strings = config.smiley_strings,
		columns = config.emoticons_columns || 8,
		i;

		//alert('CKEditor emoticon dialog, emoticons.size = ' + emoticons.size);
	/**
	...
	var size = emoticons.length;
	for ( i = 0 ; i < size ; i++ )
	{
		var txt = emoticons[i].txt,
		    img = emoticons[i].img,
		    alt = emoticons[i].alt,
		    idx = emoticons[i].idx;
		...
		html.push(
			'<td class="cke_dark_background cke_centered" style="vertical-align: middle;">' +
				'<a href="javascript:void(0)" role="option"',
					...
					'>' +*/
					'<span id="' + smileyLabelId + '" class="cke_voice_label">' + alt  + '</span>' +
				'</a>',
 			'</td>' );
