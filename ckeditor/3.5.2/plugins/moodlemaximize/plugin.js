...
(copied/adapted  from maximize plugin)
*/
...
(function()
{
	...
	{
		var dock = M.editor_ckeditor.Y.one('#dock');
		var width = dock && dock.getStyle('display') != 'none' && dock.getComputedStyle('width') || 0;
		//alert('dock : ' + dock + ', dock_width: ' + width);
		return parseInt(width);
	}

	CKEDITOR.plugins.add( 'moodlemaximize',
	{
		...
		init : function( editor )
		{
			...
			function resizeHandler()
			{
				var viewPaneSize = mainWindow.getViewPaneSize(),
				    dock_width = getDockWidth();
				shim && shim.setStyles( { width : (viewPaneSize.width - dock_width) + 'px', height : viewPaneSize.height + 'px' } );
				editor.resize( viewPaneSize.width - dock_width, viewPaneSize.height, null, true );
			}

			// Retain state after mode switches.
			var savedState = CKEDITOR.TRISTATE_OFF;

			editor.addCommand( 'mmaximize',
				{
					...
					exec : function()
					{
						...
						if ( this.state == CKEDITOR.TRISTATE_OFF )		// Go fullscreen if the state is off.
						{
							...
							var dock_width = getDockWidth();

							// Scroll to the top left (IE needs some time for it - #4923).
							CKEDITOR.env.ie ?
								setTimeout( function() { mainWindow.$.scrollTo( dock_width, 0 ); }, 0 ) :
								mainWindow.$.scrollTo( dock_width, 0 );
			...
			editor.ui.addButton( 'Mmaximize',
				{
					label : lang.maximize,
					icon: this.path + 'maximize.png',
					command : 'mmaximize'
				} );
			...
			editor.on( 'mode', function()
				{
					var command = editor.getCommand( 'mmaximize' );
